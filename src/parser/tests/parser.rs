...
fn parse_variable_tag_lit() {
    let ast = parse("{{ 2 }}{{ 3.15 }}{{ \"hey\" }}{{ true }}").unwrap();
    assert_eq!(
        ast[0],
        Node::VariableBlock(WS::default(), Expr::new(ExprVal::Int(2)))
    );
    assert_eq!(
        ast[1],
        Node::VariableBlock(WS::default(), Expr::new(ExprVal::Float(3.15)))
    );
    assert_eq!(
        ast[2],
    ...
    assert_eq!(
        ast[3],
        Node::VariableBlock(WS::default(), Expr::new(ExprVal::Bool(true)))
    );
}
...
fn parse_variable_tag_simple_negated_expr() {
    ...
    assert_eq!(
        ...
        Node::VariableBlock(
            WS::default(),
            Expr::new_negated(ExprVal::Ident("id".to_string()))
        )
    );
}
...
fn parse_variable_tag_global_function() {
    ...
    assert_eq!(
        ...
        Node::VariableBlock(
            WS::default(),
            Expr::new(ExprVal::FunctionCall(FunctionCall {
                name: "get_time".to_string(),
                args
            },))
        )
...
fn parse_raw_tag_with_ws() {
    ...
    assert_eq!(
        ast[0],
        Node::Raw(start_ws, "    yaml_test:     ".to_string(), end_ws)
    );
}
...
fn parse_simple_macro_definition() {
    ...
    args.insert(
        "b".to_string(),
        Some(Expr::new(ExprVal::String("hello".to_string()))),
    );
...
fn parse_string_concat_can_merge() {
    ...
    assert_eq!(
        ...
        Node::VariableBlock(
            WS::default(),
            Expr::new(ExprVal::String("hellohey".to_string()))
        ),
    );
}
#[test]
